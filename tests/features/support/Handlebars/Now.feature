Feature: Handlebars helpers
  ...
  Scenario: Default date generation
    Given a http mock file for "/handlebars/now/default"
      ```
      HTTP/1.1 200 OK
  ...
  Scenario: Date generation with a specified format
    Given a http mock file for "/handlebars/now/format"
      ...
      ```
    When the http url "/handlebars/now/format" is called
